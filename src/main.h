...
static const uint256 hashGenesisBlockOfficial("00000c2c71c0a0df4da4fa6d5756dc6b0c8709fc9c4e58973b330bb96900e2b4");
#ifdef TESTING
static const uint256 hashGenesisBlockTestNet("000009dedd6681804314c35707810c3e6e59b73c6db85908df128348059e796e");
#else
static const uint256 hashGenesisBlockTestNet("00000f98d72924bfa721e22fa3e42a5084855e81334fe8d3f3ef01f9f2f7c5dc");
#endif
...
public:
    ...
    std::map<uint32_t, CBlockIndex*> mapAssetsPrev;
    // bcexchange: asset votes results for this block
    std::map<uint32_t, CAsset> mapAssets;

    // bcexchange: rewarded signer
    CSerializableTxDestination rewardedSigner;
    ...
    {
        return rewardedSigner.GetTxDestination();
    }
    bool CalculateRewardedSigner();
    ...
    bool GetEffectiveAssets(std::map<uint32_t, CAsset>& mapEffectiveAssets) const
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(const uint32_t, CBlockIndex*)& item, pEffectiveIndex->mapAssetsPrev)
            item.second->GetBlockVotedAsset(item.first, mapEffectiveAssets[item.first]);

        // Copy any assets that are voted in this block
        BOOST_FOREACH(const PAIRTYPE(const uint32_t, CAsset)& item, pEffectiveIndex->mapAssets)
    ...
    bool GetVotedAsset(uint32_t nAssetId, CAsset& asset) const
    {
        ...
        std::map<uint32_t, CBlockIndex*>::const_iterator it = mapAssetsPrev.find(nAssetId);
    ...
    {
        std::map<uint32_t, CAsset>::const_iterator it = mapAssets.find(nAssetId);
        if(it != mapAssets.end())
        {
            asset = it->second;
            return true;
...
public:
    ...
    (
        ...
        if (IsProofOfStake())
        {
            ...
            if (nProtocolVersion >= PROTOCOL_V4_0 && nVersion >= 5000300) // v5.0.3
                READWRITE(rewardedSigner);
            else if (fRead)
                const_cast<CDiskBlockIndex*>(this)->rewardedSigner = CNoDestination();
        }
        else if (fRead)
        {
