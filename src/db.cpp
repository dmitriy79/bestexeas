...
bool CTxDB::LoadBlockIndex()
{
    ...
    loop
    {
        ...
        if (strType == "blockindex" && !fRequestShutdown)
        {
            ...
            pindexNew->signerRewardVoteResult = diskindex.signerRewardVoteResult;
            pindexNew->mapAssets              = diskindex.mapAssets;
            pindexNew->rewardedSigner         = diskindex.rewardedSigner;
    ...
    // bcexchange: Have rewarded signers been serialized in block index yet?
    bool fRewardedSignerSerialized;
    Read(string("fRewardedSignerSerialized"), fRewardedSignerSerialized);
    if (!fRewardedSignerSerialized)
        printf("Rewarded signers will be recalculated and saved into the block index\n");

    CTxDB writedb;
    CBlockIndex* pindexFork = NULL;

    //check whether current chain includes a block not matching the checkpoints
    for (CBlockIndex* pindex = pindexGenesisBlock; pindex; pindex = pindex->pnext)
    {
        ...
        {
            printf("ERROR: Block %d doesn't match checkpoint\n", pindex->nHeight);
        ...
        if (pindex->mapMoneySupply.empty() || pindex->mapTotalParked.empty() || GetBoolArg("-rebuildsupply", false))
        {
            ...
            BOOST_FOREACH(CTransaction& tx, block.vtx)
            {
                ...
                else
                {
                    ...
                    MapPrevTx mapInputs;
                    if (!tx.FetchInputs(writedb, mapQueuedChanges, true, false, mapInputs, fInvalid))
            ...
            if (!writedb.WriteBlockIndex(CDiskBlockIndex(pindex)))
                return error("CTxDB::LoadBlockIndex() : Rebuilding money supply: Failed write new block index");
        }

        if (!fRewardedSignerSerialized)
        {
            if (!pindex->CalculateRewardedSigner())
                return error("CTxDB::LoadBlockIndex() : Unable to calculate rewarded signer");

            if (pindex->rewardedSigner.GetTxDestination().which() != 0)
                if (!writedb.WriteBlockIndex(CDiskBlockIndex(pindex)))
                    return error("CTxDB::LoadBlockIndex() : Failed to write rewarded signer");
        }
    ...
    if (!fRewardedSignerSerialized) {
        fRewardedSignerSerialized = true;
        if (!writedb.Write(string("fRewardedSignerSerialized"), fRewardedSignerSerialized))
            return false;
    }
    ...
    if (pindexFork)
    {
        ...
        // Limit to 1000 blocks changes because large reorganizations take too much memory
        while (pindexBest != pindexFork)
        {
            CBlockIndex* pindexTarget = pindexBest;
            for (int i = 0; i < 1000; i++)
            {
                if (pindexTarget == pindexFork)
                    break;
                pindexTarget = pindexTarget->pprev;
                if (!pindexTarget)
                    return error("Moved past genesis block");
            }
            CBlock block;
            if (!block.ReadFromDisk(pindexTarget))
                return error("LoadBlockIndex() : block.ReadFromDisk failed");
            block.SetBestChain(writedb, pindexTarget);
        }
    }
