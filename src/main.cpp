...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    if (!Checkpoints::CheckHardened(pindex->nHeight, pindex->GetBlockHash()))
        return error("Block doesn't match checkpoint");
...
bool CBlockIndex::CalculateRewardedSigner()
{
    CTxDestination addressSigner;
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    if (!pindexNew->CalculateRewardedSigner())
        return error("Unable to calculate rewarded signer");
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nModifierInterval = 3;
#else
        bnInitialHashTarget = CBigNum(~uint256(0) >> 21);
#endif
    }
...
#else
            assert(block.hashMerkleRoot == uint256("0xddbfca6274429d3c4f56a77057386e1c26109e8d944ca893572311759a5d1cc1"));
#endif
